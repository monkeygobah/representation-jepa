...
LOG_DIR="_logs/baselines/${SIZE_DIR}_imagenet_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

BATCH_LOG="$LOG_DIR/batch.log"
SUMMARY_TSV="$LOG_DIR/summary.tsv"

mapfile -t CONFIGS < <(find "$CONFIG_DIR" -maxdepth 1 -type f -name '*imagenet.yaml' | sort)
if [[ ${#CONFIGS[@]} -eq 0 ]]; then
  echo "No imagenet configs found in $CONFIG_DIR" >&2
  exit 1
fi

printf "config\tstatus\texit_code\tlog_path\n" > "$SUMMARY_TSV"

echo "Starting imagenet suite" | tee -a "$BATCH_LOG"
...
echo "Finished imagenet suite" | tee -a "$BATCH_LOG"
