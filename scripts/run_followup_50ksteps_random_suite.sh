...
DEFAULT_LOG_BASE="$ROOT_DIR/_logs/followup_50ksteps_random"
DEFAULT_PATTERN='*random-50ksteps.yaml'
...
Launch the 50k-step random-init follow-up suite sequentially with torchrun and
...
SUITE_NAME="followup_50ksteps_random"
...
if [[ "$DETACH" -eq 1 ]]; then
  ...
  PID=$!
  echo "Started random-init follow-up suite in background."
  echo "PID: $PID"
...
if [[ "$FAILURES" -gt 0 ]]; then
  echo "Random-init follow-up suite finished with $FAILURES failed runs. See $SUMMARY_TSV"
  exit 1
fi

echo "Random-init follow-up suite finished successfully. See $SUMMARY_TSV"
