...
DEFAULT_CONFIG_DIR="$ROOT_DIR/configs/baselines/100k"
DEFAULT_LOG_BASE="$ROOT_DIR/_logs/baselines"
...
Launch a baseline suite sequentially with torchrun and store one log per config.
The script detaches by default so the batch keeps running in the background.
...
if [[ "$DETACH" -eq 1 ]]; then
  ...
  PID=$!
  echo "Started baseline suite in background."
  echo "PID: $PID"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  if torchrun --nproc_per_node="$NPROC_PER_NODE" train_ssl.py --cfg "$cfg" > "$run_log" 2>&1; then
    status="ok"
    exit_code=0
...
if [[ "$FAILURES" -gt 0 ]]; then
  echo "Baseline suite finished with $FAILURES failed runs. See $SUMMARY_TSV"
  exit 1
fi

echo "Baseline suite finished successfully. See $SUMMARY_TSV"
