...
LOG_DIR="_logs/baselines/${SIZE_DIR}_random_${TIMESTAMP}"
mkdir -p "$LOG_DIR"

BATCH_LOG="$LOG_DIR/batch.log"
SUMMARY_TSV="$LOG_DIR/summary.tsv"

mapfile -t CONFIGS < <(find "$CONFIG_DIR" -maxdepth 1 -type f -name '*random.yaml' | sort)
if [[ ${#CONFIGS[@]} -eq 0 ]]; then
  echo "No random configs found in $CONFIG_DIR" >&2
  exit 1
fi

printf "config\tstatus\texit_code\tlog_path\n" > "$SUMMARY_TSV"

echo "Starting random suite" | tee -a "$BATCH_LOG"
...
for cfg in "${CONFIGS[@]}"; do
  ...
  if CUDA_VISIBLE_DEVICES="$GPU_SET" torchrun --master_port="$MASTER_PORT" --nproc_per_node="$NPROC_PER_NODE" scripts/train_ssl.py --cfg "$cfg" > "$run_log" 2>&1; then
    status="ok"
    exit_code=0
...
echo "Finished random suite" | tee -a "$BATCH_LOG"
